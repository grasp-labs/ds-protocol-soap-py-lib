...
def test_connect_cookie_session_stores_login_result_on_requests_cookie_jar(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    It stores returned session ids on the data-service host in the cookie jar.
    """
    service = make_service(auth_type=AuthType.COOKIE_SESSION, auth_test_method=None)
    fake_data_client = ZeepClientStub()
    ...
    assert cookie_jar.get("ASP.NET_SessionId", domain="example.com") == "session-id"


def test_connect_cookie_session_sets_cookie_for_data_service_host(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    It attaches the session cookie to the data WSDL host even when login only set it on the auth host.
    """
    service = make_service(auth_type=AuthType.COOKIE_SESSION, auth_test_method=None)
    ...
    fake_data_client.transport.session.cookies = cookie_jar  # type: ignore[assignment]
    fake_auth_client = ZeepClientStub(service=ZeepService(responses={"Login": "data-host-session"}))
    monkeypatch.setattr(service, "_init_client", lambda: fake_data_client)
    monkeypatch.setattr("ds_protocol_soap_py_lib.linked_service.soap.zeep.Client", lambda **kwargs: fake_auth_client)
    service.connect()
